...
cam_interface = None  # global variable for camera interface
inference_interface = None  # global variable for inference interface

def init():
    global cam_interface, inference_interface
    ...
    REX.TraceInfo(f"Initialized camera and inference with runtime: {runtime_name}, model: {model_name}")
...
def main():
    ...
    detail = str(int(REX.p3.v)) if isinstance(REX.p3.v, (bool, float, int)) else REX.p3.v
    ...
    image_path = None
    if cam_interface is not None:
        image_path = cam_interface.capture_frame(filename, sequence_no)

    # Run inference (either on captured image or sample data)
    if inference_interface is not None:
        if image_path and os.path.exists(image_path):
            inference_interface.run_inference_on_image(image_path, start_time)
        else:
            inference_interface.run_inference(start_time)  # Use sample data if no image is available

class CamInterface:
    def __init__(self, save_dir="images", sequence_no="0"):
        # Initialize camera
        REX.TraceInfo(f"Initializing camera")
        self.camera = None
        try:
            self.camera = pylon.InstantCamera(pylon.TlFactory.GetInstance().CreateFirstDevice())
            REX.TraceInfo(f"Using camera: {self.camera.GetDeviceInfo().GetModelName()}")
            if int(REX.p4.v) == 0:
                self.camera.Open()
                ...
                self.camera.ExposureTime.SetValue(float(REX.p5.v))
                self.camera.GainAuto.SetValue("Off")
                self.camera.Gain.SetValue(float(REX.p6.v))
                self.camera.BalanceWhiteAuto.SetValue("Once")
                self.camera.PixelFormat.SetValue("BGR8")
                self.camera.Close()
        except Exception as e:
            REX.TraceError(f"Error initializing camera: {e}")
            self.camera = None
    ...
    def capture_frame(self, filename="image.png", sequence_no="0"):
        """ Open camera, capture frame and save to disk. """
        self.update_path(sequence_no)

        if self.camera is None:
            REX.TraceError("Camera not available.")
            return None
        if self.camera.IsGrabbing():
            REX.TraceError("Camera is already grabbing in another program.")
            return None
        ...
        if grab_result.GrabSucceeded():
            ...
            if len(img.shape) == 2:  # Grayscale image
                ...
                pass  # RGB8 is also fine
            else:
                REX.TraceError("Image format not supported by OpenCV.")
                return None

            savepath = os.path.join(self.save_path, filename)
            success = cv2.imwrite(savepath, img)
            if not success:
                REX.TraceError(f"Failed to save image: {savepath}")
                REX.TraceError(f"Image data type: {img.dtype}, shape: {img.shape}")
                savepath = None
            else:
                REX.TraceInfo(f"Image saved to: {savepath}")
        grab_result.Release()
        self.camera.Close()
        return savepath
...
class InferenceInterface:
    def __init__(self, save_dir="results", runtime="onnxruntime", model_name="mobilenetv2"):
        ...
        if not os.path.exists(self.results_file):
            with open(self.results_file, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['Runtime', 'Model', 'Image', 'Inference Time (ms)', 'Total Time (ms)', 'Top Prediction', 'Probability'])
    ...
    def preprocess_image(self, image_path):
        ...
        try:
            ...
            if img is None:
                REX.TraceError(f"Failed to read image: {image_path}")
                return None
            ...
            # Transpose to NCHW format for ONNX models
            img = img.transpose(2, 0, 1)

            # Add batch dimension
            img = np.expand_dims(img, axis=0)

            REX.TraceInfo(f"Preprocessed image to shape: {img.shape}")
            return img

        except Exception as e:
            REX.TraceError(f"Error preprocessing image: {e}")
            return None

    def run_inference(self, start_time):
        """Run inference with sample data and measure time"""
        if self.model is None:
            REX.TraceError("Model not loaded")
            return

        try:
            REX.TraceInfo(f"Running inference with {self.runtime} on {self.model_name} using sample data")
            ...
            self.log_result(inference_time, total_time, top_predictions)

        except Exception as e:
            REX.TraceError(f"Error during inference: {e}")

    def run_inference_on_image(self, image_path, start_time):
        """Run inference on a captured image and measure time"""
        if self.model is None:
            REX.TraceError("Model not loaded")
            return

        try:
            REX.TraceInfo(f"Running inference with {self.runtime} on {self.model_name} using image: {image_path}")

            # Preprocess the image
            input_data = self.preprocess_image(image_path)
            if input_data is None:
                REX.TraceError("Failed to preprocess image, using sample data instead")
            ...
            if self.runtime == 'onnxruntime':
                if output and len(output) > 0:
                    ...
                    # Log the predictions
                    REX.TraceInfo(f"Top predictions for image {image_name}:")
                    for i, (class_id, class_name, prob) in enumerate(top_predictions):
                        REX.TraceInfo(f"  {i+1}. Class {class_id}: '{class_name}' with probability {prob:.4f}")
    ...
    def log_result(self, inference_time, total_time, top_prediction=None):
        ...
        try:
            ...
            with open(self.results_file, 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([self.runtime, self.model_name, image_name, f"{inference_time:.2f}", f"{total_time:.2f}", top_class, f"{top_prob:.4f}"])

...
python -c "import json, os; config_path = os.path.join(r'%ROOT_DIR%', 'configs', 'run_config.json'); config = json.load(open(config_path)); config['group'] = '%GROUP_NAME%'; config['mode'] = 'local'; json.dump(config, open(config_path, 'w'), indent=4)"

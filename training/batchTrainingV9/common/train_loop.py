...
def model_fn(model_class_name: str, sequence_length: int, output_dim: int):
    ...
    if model_class_name == "BigEfficientVideoModel":
    ...
    elif model_class_name == "PretrainedVisionTransformer":
        return ModelClass(
            output_dim=output_dim,
            seq_len=sequence_length,
            finetune_vit=False,
        ...
        return ModelClass(output_dim = output_dim, sequence_length=sequence_length)
...
def train_run(config_dict=None):
    ...
    run_config = load_config(os.path.join(config_dir, 'run_config.json'))
    ...
    if config_dict:
        run_config.update(config_dict)

    # Extract parameters from run_config ('param_name', 'default_value')
    model_name = run_config.get('model', 'BigEfficientVideoModel')
    ...
    mode = run_config.get('mode', 'local')
    resume_from = run_config.get('resume', None)
    group = run_config.get('group', f"default_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    ...
    accelerator.init_trackers(
        ...
        init_kwargs={"wandb": {
            "entity": entity,
            "group": group,
            "tags": [model_name.lower()] + label_columns + [str(seq_length)] + ["V9"] + ["additional_metrics"],
            "config": run_config
        }}
    )
    ...
    model = model_fn(model_name, seq_length, len(label_columns))
